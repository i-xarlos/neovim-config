  ...
  config = function()
    ...
    conform.setup({
      ...
      format_on_save = function(bufnr)
        ...
        if stats.size > max_filesize_format then
          vim.notify("File too large for automatic formatting", vim.log.levels.INFO)
          return
        end
        ...
        if line_count > max_lines_format then
          vim.notify("File has too many lines for automatic formatting", vim.log.levels.INFO)
          return
        end
        ...
        then
          vim.notify("Large TypeScript/JavaScript file - skipping automatic formatting", vim.log.levels.INFO)
          return
        end
        ...
        local function on_format(err)
          if err and err:match("timeout$") then
            ...
            vim.notify(
              "Slow formatting detected for " .. filetype .. ". Disabled for future operations.",
              vim.log.levels.WARN
            )
          end
        end
    ...
    vim.keymap.set({ "n", "v" }, "<leader>f", function()
      ...
      if ok and stats and stats.size > max_filesize_format then
        vim.notify("File too large for formatting: " .. math.floor(stats.size / 1024) .. "KB", vim.log.levels.WARN)
        return
      end
