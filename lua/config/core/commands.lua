...
vim.api.nvim_create_autocmd("VimEnter", {
  callback = function()
    ...
    vim.api.nvim_create_user_command("StyleLua", function()
      ...
      local output = vim.fn.system({ "stylua", "--config-path", vim.fn.expand("~/.config/nvim/utils/linter-config/stylua.toml"), file })
      
      if vim.v.shell_error ~= 0 then
        vim.notify("StyleLua error: " .. output, vim.log.levels.ERROR)
      else
        vim.notify("Successfully formatted with StyleLua", vim.log.levels.INFO)
    ...
    vim.api.nvim_create_user_command("StyleLuaConfig", function()
      local file = vim.fn.expand("~/.config/nvim/utils/linter-config/stylua.toml")
      if vim.fn.filereadable(file) == 1 then
        vim.cmd.edit(file)
      else
        vim.notify("StyleLua config file not found: " .. file, vim.log.levels.ERROR)
      end
